...
1. 打开 [Releases](../../releases) 页，下载 `搜分名单查询工具.zip`（约 28 MB，**唯一的附件**）
2. 解压到一个空文件夹——里面有成品 HTML 和 3 个启动器
3. 双击 `启动本地服务.cmd`（推荐）或直接双击 `搜分名单查询工具.html`
4. 把名单文件（或整个 zip）拖进页面 → 输入姓名 → 点「开始解析」

> 成品体积大是因为 OCR 模型与全部第三方库都内嵌在文件里——这正是它离线可用的原因。
> 两个启动器都会用 `--force_high_performance_gpu` 让 WebGPU 走独立显卡：
>
> - `启动本地服务.cmd`——起 `127.0.0.1:18010` 本地服务再打开，pdf.js / Tesseract 功能最完整（推荐）
> - `直接打开页面.cmd`——不起服务，直接 `file://` 打开，最省事
>
> 没装 Python 也没关系：`启动本地服务.cmd` 会自动改用附带的 `本地服务_无Python.ps1`。
